...
handle([?Atom(r), ?Atom(RecordName)], ?S(Line, map, {Var, KVs}), State) ->
    {EVar, State1} = fn_to_erl:ast_to_ast(Var, State),
    {Items, State2} = fn_to_erl:state_map(fun to_record_field/2, KVs, State1),
    R = {record, Line, EVar, RecordName, Items},
    {R, State2};
% #r.<atom> <map>
handle([?Atom(r), ?Atom(RecordName)], ?S(Line, map, KVs), State) ->
    {Items, State1} = fn_to_erl:state_map(fun to_record_field/2, KVs, State),
...
to_record_field({kv, Line, Key, Val}, State) ->
    {{EKey, EVal}, State1} = fn_to_erl:kv_to_ast(Key, Val, State),
    R = {record_field, Line, EKey, EVal},
    {R, State1};
to_record_field(Other, State) ->
    Line = element(2, Other),
    State1 = fn_to_erl:add_error(State, bad_record_field_init, Line,
                       fn_to_erl:expected_got("initialization", {ast, Other})),
